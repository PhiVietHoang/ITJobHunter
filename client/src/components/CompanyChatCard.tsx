...
const ChatCard = () => {
    ...
    const user = useSelector((state: RootState) => state.employerAuth.company)
    ...
    React.useEffect(() => {
        if (
            ...
            typingOn &&
            receiver?._id === TyperID?.companyId
        ) {
            setTyping(true)
        } else {
            setTyping(false)
        ...
        <>
                        ...
                        <div className='bg-white overflow-hidden rounded-full w-12 h-12 flex items-center justify-center border border-gray-200 text-black -xl'>
                            ...
                            {!receiver.avatar && (
                                <div className='flex items-center justify-center'>
                                    <span>{receiver.name.slice(0, 1)}</span>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            >
                {messages.map((message, i) => {
                    ...
                    const chatStyle = isSender ? 'order-2 item-end' : 'items-start order-1'
